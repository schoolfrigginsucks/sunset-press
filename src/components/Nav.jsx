...
export default function Nav() {
  ...
  /* Brief pulse on the badge whenever something lands in the cart. */
  useEffect(() => {
    if (!addedAt) return
    setPulse(true)
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-40 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
        scrolled
          ? 'border-b border-ink-900/[0.07] bg-cream-100/85 backdrop-blur-xl'
    ...
    >
      ...
      >
        <a href="#top" className="text-lg text-ink-900" aria-label="Sunset Press — home">
          <Logo />
        </a>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {count > 0 && (
              <span
                className={`absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full sunset-gradient px-1 text-[0.68rem] font-semibold text-white transition-transform duration-300 ${
      ...
      <div
        id="mobile-nav"
        className={`overflow-hidden border-t border-ink-900/[0.07] bg-cream-100/95 backdrop-blur-xl transition-[max-height,opacity] duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] md:hidden ${
